...
# Exact validated cumulative artifact 10839036061 / 5a64c344.
MEASURED_ELF_END = 0x800BC150
...
def prove_layout() -> tuple[int, int, int]:
    ev = parse_macros()
    raw_q1 = ev.name("HCOMP_RAW_PALETTE_QUEUE1")
    raw_q2 = ev.name("HCOMP_RAW_PALETTE_QUEUE2")
    section_q1 = ev.name("SECTION_QUEUE1")
    section_q2 = ev.name("SECTION_QUEUE2")
    section_size = ev.name("SECTION_SIZE")

    if raw_q1 != 0xA00EF000 or raw_q2 != 0xA00EF800:
        raise AssertionError(f"raw palette ABI drift: 0x{raw_q1:X} 0x{raw_q2:X}")
    ...
    if EVENT_Q2[1] != raw_q1:
        raise AssertionError("event area does not end exactly at raw-palette Q1")
    ...
    raw_cached = cached(raw_q1)
    total_gap = raw_cached - MEASURED_ELF_END
    headroom = lowest_cached - MEASURED_ELF_END
    if total_gap != 0x32EB0:
    ...
    return section_capacity, event_capacity, headroom
...
def main() -> int:
    section_capacity, event_capacity, headroom = prove_layout()
