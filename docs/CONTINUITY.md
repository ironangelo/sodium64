...
- Status: **E2B CARRIER VALIDATED / COMPACT BASE FAILURE MEASURED / MAIN-SIDE SWITCH SUPPORTED / ONE-CONSTANT FIX NEXT**.
- Immediate next action: patch **only** the first-frame compact Color Image base in `src/rsp_main.S` from `0x000E2E80` to validated global-y=16 base **`0x000E1D00`**. Do not change the pass-boundary main restore, fence behavior, guest, classifier or section oracle. Re-run exact semantic capture; then require carrier geometry unchanged plus compact scratch 2,048× green / 192× sentinel with both guards intact and main rows 8..15 2,048× red.
