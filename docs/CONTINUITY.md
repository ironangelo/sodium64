...
**2A LAYOUT CONTROL — IMPLEMENTED / NOT YET MEASURED:** exact HEAD **`e2b1979a4bda3cf853bd7c5c029dab4d0b9c481e`**, parent safe HEAD `225859b1...`. Direct compare is exactly one file `src/apu_address.S`, **+10/-0**. The helper is appended at file end and **no existing `apu_read8` call target is changed**, so the helper is unreachable during emulation. Any 2A frame-budget movement is therefore code/layout/link effect, not map-lookup optimization.
...
## RESUME HERE
1. Read CI/profile state for exact **2A HEAD `e2b1979a...`**. Record Build/Validate + representative Gothicvania ares run IDs/artifacts immediately. Compare 2A complete virtual frame budget to baseline16 44/60; helper is unused, so any material movement is layout confound.
