...
# Clean current-master window layout, independently audited.
PRIO_CHECKS = 0xE7C
WIN_BOUNDS = 0xE84
WIN_COUNT = 0xE8A
...
def prove_layout_capacity() -> None:
    ...
    if six_span_bytes_end != WIN_COUNT:
        raise AssertionError("WIN_COUNT must follow six span bytes")
    if not (WIN_COUNT < OVERLAY_MAIN_SRC):
        raise AssertionError("WIN_COUNT overlaps overlay source pointer")
