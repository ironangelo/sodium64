...
### ACTIVE / RESUME HERE — E2b branch exists; validate carrier before interpreting target pixels
...
- **Repo reconciliation:** `phase4/gate-c-h-comp-e2b-real-target-switch` already exists and is **2 commits ahead** of the E2a head. Current exact head is **`0f41914ee000181210192926d87ff22fa2397f85`**. Repo evidence supersedes the earlier “create child branch next” handoff.
- Parent **`af1ad807b3a2890e8f60501e2bffc2e834f4d65a`** (`proof: route first real sub traversal to compact color target`) changes the proof runtime: frame start binds Color Image to compact RGB16 base `0x000E2E80 = 0xE4000 - 8*560`; at the existing `srl s7,s7,8` dual-screen boundary, only when `k0==0`, it restores `FRAMEBUFFER(sp) - 8*560` before the second traversal. The large E2a synthetic strip body/helpers were removed; `next_frame` retains Sync Full + DP-idle fencing and the existing status/framebuffer marker.
- Head **`0f41914...`** (`proof: add E2b no-shared 8-line carrier guest`) changes only the deterministic guest relative to that parent: **BG1 opaque red / TM=0x01 main-only**, **BG2 opaque green / TS=0x02 sub-only**, TMW/TSW=0, no color math. It uses direct HDMA mode 0 to WH0 with 8 visible writes of `0x00` then `0x01` for the remaining visible lines; windows remain disabled, so the write is intended only to create an urgent section boundary.
- **SUPERSEDED planning detail:** the earlier VIRQ/WH0 + checker/backdrop carrier precommit is no longer the candidate implemented in the repo. The actual HDMA + solid-red/green carrier is acceptable as a cleaner discriminator only if its section geometry is first measured. Do not mix the obsolete oracle with the candidate.
- **Important ordering debt:** target-switch runtime was implemented before the carrier's exact 8-line section was dynamically proven. Therefore **do not interpret compact/main pixel results yet**. First validate the carrier geometry; if it is not exactly 8 rows, the compact target may be overrun and any pixel evidence is invalid.
- The existing capture harness already creates a clean epoch between halted frames and host-initializes `0xA00E4000..0xA00E517F` to RGB16 sentinel `0x55AA` with prefix/suffix guards `0xC3/0x3C`. Thus E2b can measure compact overwrite/guards later without re-adding E2a resident initialization helpers.
- **Next controlled batch is host/capture-only:** capture both section queues and classify the carrier before any further RSP edit. Existing section ABI already pins `TM@0x3A` and `SPLIT_LINE@0x3F`; `TS@0x39` follows the contiguous `ts,tm` section layout. Require an actual queued first section with `TM=0x01`, `TS=0x02`, `SPLIT_LINE=8` (and record both queues for stale/current disambiguation). Keep runtime byte-identical.
- **After carrier validation only**, E2b pixel oracle for the actual solid carrier becomes: compact first band active x=12..267 = **2,048 × green `0x07C1`**, border 192 × sentinel `0x55AA`, guards intact; corresponding main first band active x=12..267 = **2,048 × red `0xF801`**, with zero green. Main border contents are not part of this discriminator.
- Current generic **Build and Validate run `35658495855`** for `0f41914...` is in progress at this checkpoint. Generic green, when/if obtained, will prove build/smoke only, not the E2b semantic contract.
- Existing E2a semantic workflow on this branch is still named/filtered for E2a; it has **not** validated E2b. Retargeting the semantic workflow belongs with the capture-only discriminator once its classifier no longer expects E2a synthetic outputs.
- **E2b pass meaning remains narrow:** real existing `next_layer` dual traversal can separate a no-shared sub operand into a compact Color Image and restore main for the second traversal in a true 8-row section. Shared layers are E2c; >8-row band slicing, compositor arithmetic, raw-brightness plumbing, color windows, throughput, PR #13 overlays and hardware cadence remain out of scope.
- Status: **IMPLEMENTED CANDIDATE / NOT YET VALIDATED**. Immediate next action: add the section-queue capture/classifier + E2b semantic workflow retarget as a host-only delta, run it on the exact head, checkpoint the result, and only then inspect target pixels.
