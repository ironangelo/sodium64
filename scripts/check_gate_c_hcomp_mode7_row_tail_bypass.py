#!/usr/bin/env python3
"""Classify a proof-only row-tail bypass after all 16 Mode7 tile positions."""
...
def classify(root:Path)->dict:
    ...
    mk=marker(mb)
    if mk is not None:
        classification="MODE7_ROW_TAIL_BYPASS_HCOMP_REACHED"
        meaning="all 16 tile positions ran before row-tail bypass and H-COMP executes; the baseline delay lies in the row tail or following row, not in the first-row empty tile loop alone"
        mailbox_mode=mk[0]; mailbox_norm=mk[1].hex()
    elif mm is not None:
        classification="MODE7_ROW_TAIL_BYPASS_HCOMP_NOT_REACHED"
        meaning="one heavy texture DMA occurred but H-COMP was not observed after all 16 positions and a row-tail bypass; first-row empty traversal may be sufficient for the pinned-lab observation"
    ...
    return {
        ...
        "model":model,
        "diagnostic_runtime":"one row-tail instruction substitution after the first 16 tile positions, not production renderer semantics",
...
def self_test()->None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_ROW_TAIL_BYPASS_HCOMP_NOT_REACHED"

        (p/"mailbox.bin").write_bytes(swap32(bytes((0x00,0x51,0,0,0,0,0,0))))
        (p/"guest-state.bin").write_bytes(swap32(EXPECTED_GUEST))
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_ROW_TAIL_BYPASS_HCOMP_REACHED"
