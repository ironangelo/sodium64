#!/usr/bin/env python3
"""Generate Gate-C normal-W1 OBJ span-termination diagnostic SNES ROM.

This ROM contains no commercial game data. It isolates the PR#13 H-OBJ
visible-span termination contract with no preceding windowed BG.

Visible setup:
- Mode 1, OBJ only on the main screen.
- W1 = [64, 191].
- OBJ selects normal W1.
- Outer 8x8 controls at x=16 and x=224.
- A central 16x8 marker is formed by adjacent sprites at x=120 and x=128.
- Backdrop is blue so the direct-SNES viewport remains easy to locate.
- No BG layer, subscreen, color math, HDMA, IRQ, or W2.
...
- control: left, central, and right red markers are visible.
- treatment: the central marker is masked by normal W1 while both outer
  controls remain visible.

The exact broken candidate can walk beyond the valid [0,64],[191,255] span
list after the second interval and later reinterpret non-window DMEM bytes as
bounds. Any treatment pixels from the central marker, or failure to progress,
falsify bounded traversal. This diagnostic does not test W2/combine logic,
color math, Mode7 window semantics, or general compositor correctness.
"""
...
def build_program() -> bytes:
    ...
    for value, address in (
        ...
        (0x02, 0x2125),          # WOBJSEL: OBJ uses normal W1
...
def build_assets() -> tuple[bytes, bytes, bytes, bytes, bytes]:
    ...
    for index, x in enumerate((16, 120, 128, 224)):
        offset = index * 4
        oam[offset + 0] = x
        oam[offset + 1] = 96
        oam[offset + 2] = 0
