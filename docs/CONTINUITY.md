...
- **CURRENT RUNNING experiment — SRS route v4:** `phase3/gate-b-srs-representative@7932bc0cf44722990815685ed0f1d1e4e1951d92`. Only targeted lower-platform jump timing changes from gf374..389 to **gf378..393**, with all other v3 input identical. Build/Validate **`35422018496` SUCCESS**. Direct-SFC run **`35422018484`** has already passed SRS source rebuild + semantic-symbol resolution and is currently building pinned accurate ares. **Resume by inspecting this exact run first; do not relaunch it.** Success criterion: an actual upward jump from y4195 followed by sustained x>4554, preferably further room progression. Failure criterion: jump executes but still cannot cross, or no jump despite gf378 standing state. No Sodium64 measurement until direct route qualification passes.
