#!/usr/bin/env python3
"""Classify a proof-only row-branch destination discriminator after the complete native first row."""
...
def classify(root:Path)->dict:
    ...
    mk=marker(mb)
    if mk is not None:
        classification="MODE7_ROW_BRANCH_HCOMP_REACHED"
        meaning="H-COMP executes after preserving first-row tiles and row-tail arithmetic; a taken row-backedge or its successor distinguishes this from baseline, subject to live section-bound verification"
        mailbox_mode=mk[0]; mailbox_norm=mk[1].hex()
    elif mm is not None:
        classification="MODE7_ROW_BRANCH_HCOMP_NOT_REACHED"
        meaning="one heavy texture DMA occurred but H-COMP was not observed even with forced exit at row branch; row-tail arithmetic or lab timing remains possible"
    ...
    return {
        ...
        "model":model,
        "diagnostic_runtime":"one row-branch target substitution preserving first-row arithmetic, not production renderer semantics",
...
def self_test()->None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_ROW_BRANCH_HCOMP_NOT_REACHED"

        (p/"mailbox.bin").write_bytes(swap32(bytes((0x00,0x51,0,0,0,0,0,0))))
        (p/"guest-state.bin").write_bytes(swap32(EXPECTED_GUEST))
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_ROW_BRANCH_HCOMP_REACHED"
