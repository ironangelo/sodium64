...
- **Current running harness-cleanup candidate:** `phase3/gate-b-srs-reference@dd05ba502a6c4c6b7c4de2da1384207d23ea5a2d`. One harness-only line changes tracer termination `std::exit(0) -> std::_Exit(0)`; guest/input/ROM/trace logic are unchanged. Direct-reference run **`35419188132`** and Build/Validate **`35419188123`** were in progress at this checkpoint. **Resume by inspecting these exact runs first.** If the reference is green, record artifact ID/digest and verify semantics remain identical; if it fails after all 600 rows, classify teardown only and repair harness without reopening the route conclusion.
- **Next technical Gate B action after evidence hygiene:** create a new SRS representative input route using normal controller semantics only: B appears in `pressed` on the real edge and may remain in `current` while held; do not touch player/collision state. Validate the new route on direct SFC first and require sustained progression beyond local x=154, preferably the authored transition to `a1b_stairway`. Then run safe matched Sodium64 on the same route. Add the game-owned 32-bit `frameCounter` to Sodium observability if same-frame differential alignment is needed; read bytes individually through guest virtual WRAM like the existing semantic fields.
