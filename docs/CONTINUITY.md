...
- **Immediate E2c action:** update only the host classifier from compact black `0x0001` to compact red `0xF801` while keeping `TS=TM=1`, main red, geometry and guards unchanged. Then run exact-head semantic CI. If green, E2c shared-membership closes; if red, diagnose layer traversal/cache/priority before any compositor work.
