#!/usr/bin/env python3
"""Classify a proof-only return bypass after the first heavy Mode7 tile."""
...
def classify(root:Path)->dict:
    ...
    mk=marker(mb)
    if mk is not None:
        classification="MODE7_FIRST_HEAVY_RETURN_BYPASS_HCOMP_REACHED"
        meaning="with a bypass at the first finish_tile7, H-COMP executes; first heavy RDP returned and the skipped tile/row path contributes to the baseline non-observation in this lab"
        mailbox_mode=mk[0]; mailbox_norm=mk[1].hex()
    elif mm is not None:
        classification="MODE7_FIRST_HEAVY_RETURN_BYPASS_HCOMP_NOT_REACHED"
        meaning="one heavy texture DMA occurred but H-COMP was not observed despite bypass; this alone does not prove RDP deadlock"
    ...
    return {
        ...
        "model":model,
        "diagnostic_runtime":"one branch substitution at finish_tile7, not production renderer semantics",
...
def self_test()->None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_FIRST_HEAVY_RETURN_BYPASS_HCOMP_NOT_REACHED"

        (p/"mailbox.bin").write_bytes(swap32(bytes((0x00,0x51,0,0,0,0,0,0))))
        (p/"guest-state.bin").write_bytes(swap32(EXPECTED_GUEST))
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_FIRST_HEAVY_RETURN_BYPASS_HCOMP_REACHED"
