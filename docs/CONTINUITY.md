...
- **2026-09-21 E1a batch 39 — robust-guest Z baseline ACTIVE; signatures precommitted before result:** proof head **`4a77a99744d2f8fbb643d3bb92a5969fec63e2a8`** changes only two runtime facts relative to validated robust color-control `d57437c5...`: restores E1 OtherModes `0x2F0088FF00040025` (primitive-depth select + Z update, no Z compare) and re-enables initial `Set Z Image` send. Guest remains alternating tile IDs with visually identical checkerboard tiles, already measured at exact **28,672 opaque-black + 28,672 backdrop + 9,856 border** when Z was disabled. Exact Build/Validate **`35619089380 SUCCESS`**; semantic **`35619089437`** is in pinned-ares build at checkpoint. **Precommitted signatures:** framebuffer must remain 28,672/28,672 if Z instrumentation does not corrupt color. If transparent BG holes preserve preexisting sentinel and backdrop does not write Z, expect `tag=28,672 / sentinel=38,528`. If source-supported Cycle1 backdrop inherits persistent BG primitive depth, expect `tag=57,344 / sentinel=9,856` while colors remain the full checkerboard. Any different topology requires separate classification. If inherited-backdrop signature appears, next controlled proof is a distinct backdrop primitive tag (e.g. `0x0800 -> stored 0x0400`) before fill, BG remains `0x1800 -> 0x0C00`.

- **2026-09-21 E1a guest-control batch 38 — repeated-tile issue is not an E1-slot boundary artifact; defer dedicated repair until H-COMP discriminator closes:** source comparison `master@5b713493...` vs robust-guest proof `d57437c5...` shows `rdp_tile` is the same 5-dword regular command block and `skip_upload` is byte-for-byte identical: it sends only `RDP_TILE+24 .. RDP_TILE7`, i.e. the two-dword Texture Rectangle command, with `RDP_TILE7` remaining the exact next-label boundary in both layouts. Therefore the 16×7 failure is **not explained by E1 reusing the contents after the RDP_TILE7 label**. Since alternating visually-identical tile IDs alone restores the exact 256×224 checkerboard, retain **OPEN QUESTION / COMPATIBILITY PROOF candidate:** inherited repeated-same-tile fast path may be incorrect (or expose a pinned-emulator RDP limitation). Do not repair it inside H-COMP E1 yet; a dedicated later discriminator should compare a minimal production-like master path and test whether a required RDP sync/command-range invariant is missing. For current H-COMP, robust alternating-ID guest is now the valid visual control.

- **2026-09-21 E1a guest-control batch 37 — repeated-tile fast path IDENTIFIED as cause of 16×7 truncation; robust guest control VALIDATED:** repo had advanced beyond prior RESUME HERE. Control head `04c7fdd50726535d0bfd9a35692ced9e30f404b7` (commit `proof: isolate E1 color path without Set Z Image`) had Build/Validate **`35616327552 SUCCESS`** and semantic `35616327519 FAILURE` only because the unchanged Z classifier expected tags. Artifact **`10646850520`** shows framebuffer **unchanged at 56 black + 57,288 backdrop + 9,856 border**, while depth is **67,200/67,200 sentinel**: removing residual Set-Z-Image association did not restore color. Therefore E1 depth instrumentation as a class is **REJECTED** as the cause of the tiny color topology. Next head `d57437c5e6884acfeef2767d31514e1469d26c2c` (`proof: bypass repeated-tile fast path in E1 guest`) had Build/Validate **`35617628554 SUCCESS`** and semantic `35617628477 FAILURE` only because Z remained intentionally disabled. Artifact **`10646669714`** is decisive: framebuffer becomes the exact intended full control, **28,672 opaque-black + 28,672 visible-backdrop + 9,856 border**, with depth still **67,200/67,200 sentinel**. **SUPPORTED INTERPRETATION:** the original all-tile-0 guest triggered Sodium64's regular repeated-tile fast path (`skip_upload`) and that path caused the reproducible 16×7 color truncation; changing the tilemap to avoid that path restores the full 256×224 checkerboard without changing sections/capture/Z. This is a newly exposed regular-renderer correctness issue, potentially independent Gate-C knowledge. Immediate next action: audit the exact `skip_upload` command-range semantics and preserve this bug separately from H-COMP; meanwhile use the robust guest as the valid E1 color control.

- **2026-09-21 E1a robust-Z batch 47 — original E1 depth baseline RESTORED/DISPATCHED on robust guest:** proof head **`4a77a99744d2f8fbb643d3bb92a5969fec63e2a8`**, one-file controlled delta from robust no-Z head `d57437c5...`: `rdp_init` OtherModes restored to E1 `0x2F0088FF00040025` (primitive-depth selection + Z update, no Z compare) and startup `Set Z Image` `rdp_send` restored. Alternating pixel-identical tile IDs, section diagnostics, frame-boundary capture, per-BG `prim_depth 0x1800` and end-frame fence/marker are unchanged. Exact-head runs: **Build/Validate `35619089380`** and **E1a semantic `35619089437`**, in progress. **Precommitted baseline signature:** framebuffer should remain robust full-screen **28,672 black + 28,672 backdrop + 9,856 zero**. Because backdrop rendering can inherit the persistent BG primitive depth, current one-code baseline may still produce **57,344 active `0x0C00` + 9,856 sentinel**; that would confirm Z plumbing while remaining unable to prove transparent-hole preservation. Any color regression must be investigated before introducing distinct depth codes.

- **2026-09-21 E1a guest-control batch 46 — alternating pixel-identical IDs VALIDATE robust full-screen color control; same-tile fast path causally implicated in proof runtime:** exact head `d57437c5e6884acfeef2767d31514e1469d26c2c`; Build/Validate `35617628554 SUCCESS`; semantic `35617628477 FAILURE` only because unchanged Z classifier expects tags while this control intentionally has no Z writes. Evidence artifact **`10646669714`** is decisive and matches the batch45 precommitted signature exactly: framebuffer **28,672 `0x0001` opaque black + 28,672 `0x8001` visible backdrop + 9,856 zero border/tail**, guards intact; depth **67,200/67,200 sentinel `0x55AA`, zero tags**. Raw framebuffer SHA256 **`aad0844b5a879221318185ca2bca7eadd92fa8b89f453adb4ad2cad17f247910`**; depth SHA256 `92b9fd2a5d8174c348f5a81fe315a94aa71c14e427b73edf3de51ed718fe08c1`; deterministic guest SHA256 **`1063caa98ded62be2a9f5a0c03726e05143c13e2111108327a1b5b8d9b1fe460`**. The **only** semantic guest change from the failing 56-pixel control is alternating character IDs 0/1 with byte-identical tile graphics, which prevents consecutive entries from taking `skip_upload`. **MEASURED:** the alternating-ID guest is a valid robust E1 color control. **SUPPORTED INTERPRETATION:** consecutive-same-tile / `skip_upload` behavior is causally involved in the 16×7 truncation on this proof runtime. **UNKNOWN:** whether integrated master exhibits a production correctness bug under the same condition; do not open/fix that tangent now. Decision: keep the anomaly as Gate-C renderer knowledge and resume H-COMP using the robust alternating guest. Immediate next controlled change: restore only the original E1 Z baseline (primitive-depth/Z-update OtherModes + startup Set-Z) while keeping this guest and capture harness unchanged; do not introduce distinct backdrop/BG tags until that baseline is measured.

- **2026-09-21 E1a guest-control batch 45 — alternating guest signatures precommitted:** because character IDs 0 and 1 contain byte-identical checkerboard texels, a correct 256×224 active render must remain **28,672 opaque-black `0x0001` + 28,672 visible backdrop `0x8001` + 9,856 logical capture border/tail**; the current master-OtherModes/no-Set-Z control must also remain **67,200 sentinel `0x55AA`, zero Z tags**. Full signature validates the guest as a robust E1 color control and implicates consecutive-same-tile behavior in the old ROM; exact 56-black signature rejects the `skip_upload` predicate as primary cause; any intermediate spatial/count signature is UNKNOWN and must be inspected rather than rounded into either outcome.

- **2026-09-21 E1a guest-control batch 44 — alternating guest Build/Validate GREEN:** exact proof head `d57437c5e6884acfeef2767d31514e1469d26c2c`; standard **Build and Validate `35617628554 SUCCESS`** including normal build, PROFILE build and pinned Mupen/LLE smoke (`update-release` skipped). Thus the guest-only 0/1 alternating tilemap + duplicate checkerboard tile data introduces no generic build/boot/smoke regression. Semantic `35617628477` has passed deterministic guest generation, exact proof build, wrap and state-location steps and remains in pinned-ares build at checkpoint. Its framebuffer topology is the sole current discriminator.

- **2026-09-21 E1a guest-control batch 43 — decision after alternating-tile result precommitted:** if head `d57437c5...` produces the intended full checkerboard, treat that as a **measurement/control repair first**, not an automatic production `skip_upload` fix. Persist the repeated-same-tile anomaly as Gate-C renderer evidence/open question, but immediately use the robust alternating-ID guest to resume the original E1 architecture question. The next E1 proof should restore Set-Z + primitive-depth/Z-update and use **distinct backdrop vs BG depth codes** (planned backdrop `prim_depth 0x0800 -> stored 0x0400`, BG `0x1800 -> 0x0C00`) so transparent BG holes are proven by preserving the backdrop code rather than a sentinel. If alternating IDs still yield 16×7, do not touch Z; first remove the timing-only per-BG `e1_set_prim_depth` submission to approach master regular-render command cadence.

- **2026-09-21 E1a guest-control batch 42 — alternating tile-ID guest IMPLEMENTED/DISPATCHED:** proof head **`d57437c5e6884acfeef2767d31514e1469d26c2c`**, exactly one file changed from batch41 head: `scripts/make_gate_c_h_comp_e1_z_tag.py` (+13/-7). Runtime/RSP, master OtherModes, no-Set-Z control, capture harness, workflow and classifier are unchanged. Guest now uploads two **visually identical** 2bpp checkerboard characters (32-byte character DMA) and fills the 32×32 Mode0 BG1 tilemap with alternating little-endian character IDs `0,1,0,1,...`; because row width is even each row restarts at 0. Expected framebuffer remains the same 50/50 checkerboard, but horizontally adjacent entries no longer satisfy the renderer's consecutive-same-tile `skip_upload` predicate. Exact-head runs auto-dispatched: **E1a semantic `35617628477`** and **Build/Validate `35617628554`**, both in progress. **Discriminator:** full checkerboard causally implicates the repeated-tile fast path/control choice; unchanged 16×7 rejects that path as the primary cause and requires a broader regular-rectangle/proof-scheduling control. Do not fix `skip_upload` or resume Z work until this result exists.

- **2026-09-21 E1a guest-control batch 41 — Set-Z association EXONERATED; move to repeated-tile control:** exact head `04c7fdd50726535d0bfd9a35692ced9e30f404b7`; Build/Validate `35616327552 SUCCESS`. Semantic control `35616327519 FAILURE` only because unchanged Z classifier expects tags; evidence artifact **`10646850520`** is authoritative. Framebuffer counts remain exactly **56 black + 57,288 backdrop + 9,856 border**, and raw framebuffer SHA256 **`d593f8b68fb074e8c7d6b1fbdd5d55c5d9ff7217c3907ca8e50da4d7a355ae18`** is byte-identical to batch35 control artifact `10642189992`. Depth is likewise byte-identical SHA256 `92b9fd2a...`, **67,200/67,200 sentinel, zero tags**. Therefore removing the startup `Set Z Image` command has zero color effect. Combined with batch35 (master OtherModes/no Z updates) and pinned source showing disabled primitive-depth constants are not consumed, **REJECT E1 depth-surface/Z-state instrumentation as the cause of the 16×7 guest color truncation in the pinned lab.** Immediate next action is the precommitted guest-only alternating tile-ID control from batch39; do not touch Z, H-COMP kernel or PR #13 for this discriminator.

- **2026-09-21 E1a guest-control batch 40 — duplicate DP_START/END suppression explanation REJECTED:** a tempting 16×7 explanation was that repeated `skip_upload` submissions reuse identical `RDP_TILE+24 -> RDP_TILE7` DPC ranges and might be ignored after the first. Pinned ares `rdp/io.cpp` disproves that mechanism: `DPC_START` sets `startValid`; every `DPC_END` sets `end`, copies `start -> current`, clears `startValid`, and calls `flushCommands()`. Thus the same start/end range is explicitly rearmed and rendered on every submission. **REJECTED:** address-equality/deduplication of repeated DPC submissions is not the cause in the pinned lab. The broader `skip_upload` path remains an OPEN QUESTION (coordinate/state/texture behavior), so the precommitted alternating-tile guest remains useful if batch37 preserves the 56-pixel topology.

- **2026-09-21 E1a guest-control batch 39 — repeated-tile discriminator designed, NOT IMPLEMENTED pending Set-Z result:** if semantic `35616327519` preserves the 56-black framebuffer, next guest-only control will alternate Mode0 BG1 tilemap character numbers `0,1,0,1,...` horizontally while storing **identical 2bpp checkerboard bytes** in tiles 0 and 1 and increasing initial character DMA from 16 to 32 bytes. Expected visible framebuffer remains exactly the same full-screen 50/50 checkerboard, but consecutive tile entries differ so `t6&0x1FFF != v1` every tile and regular BG bypasses `skip_upload`; cached revisits use `skip_decode -> draw_tile`, reissuing Set Texture/Load Block/Texture Rectangle. **Discriminator:** full checkerboard implicates repeated-tile `skip_upload`; unchanged 16×7 rejects it and requires looking at proof command scheduling / regular rectangle state before blaming cache. No code change yet; current no-Set-Z run remains the active question.

- **2026-09-21 E1a guest-control batch 38 — no-Set-Z head Build/Validate GREEN; Set Prim command source-inert under master OtherModes:** exact proof head `04c7fdd50726535d0bfd9a35692ced9e30f404b7`; standard **Build and Validate `35616327552 SUCCESS`** across normal build, PROFILE build and pinned Mupen/LLE smoke. Semantic `35616327519` remains in pinned-ares build at checkpoint. Pinned ares `op_set_prim_depth` only updates `constants.prim_depth/prim_dz`; renderer consults those values for primitive attributes only when `constants.use_prim_depth` is true. Batch35 restored master OtherModes with primitive-depth selection false, so the still-present per-BG `Set Prim Depth` command is semantically inert for color/depth in this lab (apart from command scheduling). **Decision:** do not add another depth-state control unless current no-Set-Z framebuffer changes; unchanged 56 topology is sufficient to move the semantic investigation to regular repeated-tile rendering while retaining the command as a known timing-only proof artifact.

- **2026-09-21 E1a guest-control batch 37 — Set-Z association control IMPLEMENTED/DISPATCHED:** proof head **`04c7fdd50726535d0bfd9a35692ced9e30f404b7`**, exactly one behavioral instruction changed from `ffee373f...`: startup `jal rdp_send` for the 8-byte `Set Z Image` command is replaced by `nop`; surrounding `li a0/a1`, DMEM proof slot, master OtherModes, deterministic guest, end-frame Sync Full/marker, section diagnostics and capture harness are unchanged, preserving code/data layout. Exact-head runs auto-dispatched: **E1a semantic `35616327519`** and **Build/Validate `35616327552`**, both in progress at checkpoint. Existing Z classifier is intentionally non-authoritative for this no-Z control; framebuffer SHA/topology is the discriminator. **Expected:** unchanged 56-black framebuffer exonerates residual Set-Z association and moves next batch to regular repeated-tile path; any color change requires isolating Set-Z/RDP surface-state interaction before guest changes.

- **2026-09-21 E1a guest-control batch 36 — residual Set-Z discriminator precommitted:** repo authority on resume is `master@5b7134930a0ca859f6aa24e54102de116948e3ed`, proof `phase4/gate-c-h-comp-e1-z-tag-proof@ffee373fcecbc032b9f8b7776fe0ddd26cd8bb86`. Batch35 already showed integrated/master OtherModes (`0x...0001`) leaves the exact 56-pixel color topology while eliminating all Z writes, so primitive-depth/Z-update bits are REJECTED as the color cause. **Next one-variable control:** preserve exact ROM, RSP/DMEM layout, master OtherModes, end-frame Sync Full/marker, capture harness and diagnostic artifacts; suppress only the startup `rdp_send` that emits the 8-byte `Set Z Image` command. Keep its `li a0/a1` instructions/DMEM slot intact so instruction/data layout is unchanged. **Discriminator:** if framebuffer remains byte-identical 56-black topology, E1 depth-surface association as a class is exonerated for color truncation and investigation moves to regular repeated-tile/render behavior. If color changes, isolate Set-Z surface association/ares state interaction before touching guest/cache. Existing Z classifier is expected to remain red because depth updates are already disabled; framebuffer artifact is authority.

- **2026-09-21 E1a guest-control batch 35 — primitive-depth/Z-update bits EXONERATED for the 16×7 color truncation:** exact control head `ffee373fcecbc032b9f8b7776fe0ddd26cd8bb86`; Build/Validate `35609056653 SUCCESS`. Semantic control `35609056650 FAILURE` only because unchanged E1 Z classifier expected tags; evidence artifact **`10642189992`** is decisive. With integrated/master OtherModes (`0x...0001`) and identical guest/capture, framebuffer is still **exactly 56 black + 57,288 backdrop + 9,856 border** (same color topology), while depth is now **67,200/67,200 sentinel `0x55AA`, zero `0x0C00` tags**. This proves the control actually disabled Z writes and simultaneously **REJECTS primitive-depth selection and DEPTH_UPDATE as the cause of the tiny color pattern**. Pinned ares source further shows Z_COMPARE is off and would not reject fragments anyway. **Next discriminator:** eliminate residual `Set Z Image` association while retaining end-frame Sync Full/marker and master OtherModes. If color still remains 56, E1 depth instrumentation as a class is exonerated and next target is repeated-tile/regular renderer behavior.
