...
- Status: **E2B TARGET SEPARATION MEASURED PASS / FINAL HOST-ASSERTION CI TODO**.
- Immediate next action: **host-only formalization batch** on the same branch/head lineage: change the capture classifier/workflow so CI asserts carrier 8/224 + compact 2,048×`0x07C1` / 192×`0x55AA` + both guards intact + main rows 8..15 2,048×`0xF801`. Do not change `src/rsp_main.S` or the guest. Re-run semantic CI; if green, mark E2b CLOSED / VALIDATED and then design E2c shared-layer discriminator.
