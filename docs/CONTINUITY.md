...
- **Current read-only qualification experiment:** v4 reaches `x4810`, which source/map correlation identifies as the left edge of the authored door structure (local center x714; +6 hitbox edge = local x720 / column45). At gf572 the player is high (`y4151`) and the column45 upper wall is solid through row7; by gf594 the same x4810 player is descending (`y4184`). The door opening is lower (rows8–10; trigger at x46/y8..10), so the 600-frame cutoff may simply end before the player falls into the opening. **Next controlled variable is observation horizon only:** extend the direct-SFC trace from 600 to 900 host frames with identical v4 ROM/input. ROM/patch identities must remain exactly unchanged. If room changes to a1b, v4 is qualified without any further input edit; if it settles again in room1, inspect the exact post-600 trajectory before changing input.
