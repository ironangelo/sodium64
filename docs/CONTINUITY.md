...
- Status: **E2B CARRIER VALIDATED / TARGET ADDRESS BUG PRECOMMITTED / TARGET-SWITCH PIXELS NEXT**.
- Immediate next action: inspect the color/framebuffer bytes already captured in artifact `10668144178` against the precommitted wrong-base signature **without changing code**. If it matches, checkpoint that measured address failure, then patch only compact Color Image base `0xE2E80 -> 0xE1D00`; leave the mid-pass switch/fence behavior otherwise unchanged.
