...
- Exact-head runs dispatched: **E2f Screen Backdrop Baseline `35678641807`** and **Build and Validate `35678641735`**. If semantic is green, the missing TM backdrop is MEASURED; only then may the runtime replay the backdrop fill at compact->main boundary.
...
- **Repair only after baseline measures:** at the existing compact->main pass boundary, after Set Color Image main and before TM layers, replay the already-prepared one-segment `RDP_FILL..RDP_WINDOW` backdrop command for the current band. Do not alter guest, band bounds, metadata encoding, arithmetic or section state.
