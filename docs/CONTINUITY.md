...
**CONTROLLED CHANGE PROPOSED:** add `apu_read8_nomap` with the same ABI and the same I/O dispatch, `apu_clock` load, and `s3` cycle decrement as `apu_read8`, but omit only the impossible `apu_map` lookup/address adjustment. First candidate redirects only statically bounded low/direct-page reads emitted by `apu_address.S`; absolute/dynamic and later operation-level reads remain on generic `apu_read8`. Keep `apu_transfer.S`, `apu_alu.S`, writes, BLOCK_SIZE, scheduler return frequency and guest cycle accounting unchanged in this first experiment.

**EXPECTED RESULT:** lower sampled cost in the APU-read path and possibly a modest frame-budget gain. **FALSIFIER:** no reproducible movement, build/semantic regression, or any evidence that a redirected call site can reach IPL-mapped space / loses I/O semantics. If neutral, reject before broadening to write paths or operation-level read substitutions.

## RESUME HERE
1. Implement M1 experiment 2 on `phase2/apu-audio-first` from exact safe HEAD **`225859b1...` / BLOCK_SIZE16**: add `apu_read8_nomap` in `src/apu.S`; redirect only proven low/direct-page `apu_read8` call targets in `src/apu_address.S`; no write-path, ALU/transfer expansion or scheduler changes. Verify diff/hygiene before CI and checkpoint new HEAD immediately.
2. Run Build/Validate + representative Gothicvania ares lab. Compare complete virtual frame budget first against baseline16 **44/60**, then sample density and generic-vs-fast read shares. If movement is small, repeat before interpreting. Reject on semantic/timing risk even if faster.
3. If the first narrow fast path is positive but small, a later separate experiment may extend the same proven-non-IPL predicate to operation-level/stack reads; do not bundle that expansion into experiment 2.
...
Resume summary: `master` is now `a2270699...` with PR #10 canonical docs integrated and verified. M0 real-N64 mean49/60, APU/audio61.83%, no VI wait. BLOCK_SIZE32 improved ares 44->48/60 but caused a paired DSP-lateness regression and is **REJECTED**. Active M1 safe HEAD remains `225859b1...` / BLOCK16. Experiment 2 is ready to implement as a narrow map-lookup elimination for proven low/direct-page reads only.