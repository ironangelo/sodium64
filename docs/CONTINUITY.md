...
- **Current Gate B experiment — SRS representative route v2 RUNNING:** `phase3/gate-b-srs-representative@982f7b1058b09077a47ad069efa03751ffadfc9e`. Controlled gameplay change from v1 is only jump phase: cadence remains one true B edge every 64 guest `frameCounter` ticks with a 16-frame hold, but phase moves from `0x08..0x17` to `0x20..0x2f` (+24 frames). Right+Run and all game/collision state are unchanged. Read-only summary filter is also repaired to exclude pre-game garbage using frameCounter/room/x sanity. Direct-SFC run **`35420515333`** is in progress building pinned accurate ares after source rebuild + semantic-symbol resolution both passed; same-head Build/Validate **`35420515316` SUCCESS**. **Question:** does the later jump phase avoid the x4554/tile-column29 stall and sustain further progression/room transition? **Falsifier:** stalls at or before v1 progress; then reject v2 and derive the next input-only route from the trace. Do not measure Sodium64 until direct SFC route qualification passes.
