...
**DOC SYNC / READY FOR PR:** `docs/m0-m1-road-sync @ 90fafc89...` reflects M0 closure, real-N64 49/60 evidence, APU/audio-first M1, conditional rather than assumed 65C816 dynarec, rejection of 32-byte APU blocks on timing evidence, and the current M1 role/limits of profiling. Full diff hygiene passed: `PROFILING.md` 9+/5-, `ROADMAP.md` 86+/53-, `ROAD_TO_1_0.md` 7+/3-, with no non-doc files. Next action is docs-only PR -> CI -> merge -> verify master.
...
## RESUME HERE
1. Open a docs-only PR from **`docs/m0-m1-road-sync @ 90fafc89...`** to `master`; require clean CI, merge if the three-doc diff remains unchanged, verify integrated master, then checkpoint continuity.
...
Resume summary: M0 real-N64 mean49/60, APU/audio61.83%, no VI wait. Baseline16=44/60 ares. Clean32 improved to48/60 twice but paired DSP-lateness proved a timing regression: BLOCK16 avg119.073/max651/0 >=672 events vs BLOCK32 avg128.338/max1218/377 >=672 events. **BLOCK_SIZE32 REJECTED.** Active M1 HEAD `225859b1...` is tree-equivalent to safe baseline16. Timer2 t0 variant is **REJECTED** as an erroneous side change. Canonical doc sync candidate is `90fafc89...`, clean three-doc diff and ready for PR.