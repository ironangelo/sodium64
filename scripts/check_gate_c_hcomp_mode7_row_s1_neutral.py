#!/usr/bin/env python3
"""Classify a proof-only neutral-s1 discriminator after the complete native first row."""
...
def classify(root:Path)->dict:
    ...
    mk=marker(mb)
    if mk is not None:
        classification="MODE7_ROW_S1_NEUTRAL_HCOMP_REACHED"
        meaning="H-COMP executes when only the row Y increment is neutralized while later row arithmetic and forced exit remain; s1 mutation is causal in this pinned lab"
        mailbox_mode=mk[0]; mailbox_norm=mk[1].hex()
    elif mm is not None:
        classification="MODE7_ROW_S1_NEUTRAL_HCOMP_NOT_REACHED"
        meaning="one heavy texture DMA occurred but H-COMP was not observed with s1 increment neutralized; s1 mutation alone is not sufficient and later row arithmetic or lab timing remains"
    ...
    return {
        ...
        "model":model,
        "diagnostic_runtime":"row-branch forced exit plus neutralized s1 increment; proof-only, not production renderer semantics",
...
def self_test()->None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_ROW_S1_NEUTRAL_HCOMP_NOT_REACHED"

        (p/"mailbox.bin").write_bytes(swap32(bytes((0x00,0x51,0,0,0,0,0,0))))
        (p/"guest-state.bin").write_bytes(swap32(EXPECTED_GUEST))
        r=classify(p)
        assert r["passed"] and r["classification"]=="MODE7_ROW_S1_NEUTRAL_HCOMP_REACHED"
...
def main()->int:
    ...
    if args.self_test:
        self_test()
        print("Mode7 row-s1-neutral classifier self-test: PASS")
        return 0
