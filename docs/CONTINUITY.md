...
**Active M1 branch:** `phase2/apu-audio-first`, current HEAD **`e2b1979a4bda3cf853bd7c5c029dab4d0b9c481e`** = experiment 2A layout control. Its parent **`225859b1d8fc0eb477a624ac76f8237667379f59`** remains the safe BLOCK_SIZE16 tree and is tree-equivalent to `c55b6b...`.
...
**HYPOTHESIS.** Real-N64 M0 sampled `apu_read8` at 6.03%; the valid ares baseline also places substantial time around the generic APU-read return path (`read_unk` 7.75% plus `apu_read8` 2.90%), so memory-read dispatch is a measured hotspot.
...
**REFINED CONTROL DESIGN:** do not modify `apu.S`. Local `apu_read8_nomap` lives at the end of `apu_address.S`. It checks the real SPC700 I/O window `0x00F0–0x00FF`; I/O tail-jumps to generic `apu_read8`, preserving the exact existing I/O/timer path. Non-I/O low reads load `apu_ram(a0)`, load `apu_clock`, subtract the same guest cycles from `s3`, and return without touching `apu_map`. Guest cycle accounting and scheduler return frequency are unchanged.

**2A LAYOUT CONTROL — IMPLEMENTED / RUNNING:** exact HEAD **`e2b1979a4bda3cf853bd7c5c029dab4d0b9c481e`**, parent safe HEAD `225859b1...`. Direct compare is exactly one file `src/apu_address.S`, **+10/-0**. The helper is appended at file end and **no existing `apu_read8` call target is changed**, so the helper is unreachable during emulation. Build and Validate run **`35226731916`** and Open Homebrew Ares Profile run **`35226731941`** were triggered for this exact SHA. Experimental question: does adding unreachable helper code/layout alone alter the baseline16 Gothicvania frame budget? Expected sane reading is ~44/60; material movement means layout is a confound and 2A, not the old baseline, must become the paired authority.

If 2A is sane/equivalent, **2B active candidate** will keep the exact helper/layout and change only proven direct/low call targets to `apu_read8_nomap`: direct, direct+X/Y, `(X)`, `(X)+`, the two low pointer-byte reads in `[dp+X]` and `[dp]+Y`, `dp,dp`'s low read, and `(X),(Y)`'s low read. Final indirect/absolute data reads stay generic. `apu_drb` 13-bit, ALU operation-level reads, stack/transfer reads, writes and BLOCK_SIZE remain outside this experiment.

**EXPECTED RESULT for 2B:** lower APU-read cost and possibly a modest frame-budget gain. **FALSIFIER:** no reproducible movement, build/semantic regression, or any redirected call capable of IPL space / losing I/O semantics. If neutral, reject before broadening scope.

## RESUME HERE
1. Read completion state for exact **2A HEAD `e2b1979a...`**, runs **`35226731916` / `35226731941`**. If successful, record artifact and complete virtual frame budget immediately; compare to baseline16 44/60. Helper is unused, so any material movement is layout confound.
2. If 2A is equivalent/sane, implement **2B** on top of it by changing only the enumerated proven-low call targets in `apu_address.S`. Verify 2A->2B diff contains only those substitutions; checkpoint exact HEAD before interpreting CI.
3. Run the same Gothicvania lab for 2B and compare complete frame budget first, then sampling shares. Repeat same SHA if movement is small/ambiguous.
...
Resume summary: `master` is `a2270699...`, canonical docs synchronized. M0 real-N64 mean49/60, APU/audio61.83%, no VI wait. BLOCK_SIZE32 improved ares 44->48/60 but caused a paired DSP-lateness regression and is **REJECTED**. Active M1 HEAD `e2b1979a...` is experiment 2A, an unused helper-only +10/-0 layout control, with Build/Validate `35226731916` and Gothicvania ares `35226731941` running.