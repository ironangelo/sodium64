...
def prove_source_abi() -> None:
    ...
    for name, src in (("main", main_src), ("mode7", mode7_src)):
        if "sll t0, t0, 26 // Move CGADSUB backdrop bit 5 into the sign bit" not in src:
            raise AssertionError(f"{name}: missing CGADSUB bit5 sign transform")
        if "bltzal t0, calc_color_window_segments" not in src:
            raise AssertionError(f"{name}: missing delay-slot-safe conditional helper call")
