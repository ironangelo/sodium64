...
def main() -> int:
    ...
    counts = collections.Counter(base_symbol_name(item) for item in symbolicated)
    total = len(samples)

    print("\nHottest sampled symbols/regions:")
    for name, count in counts.most_common(max(args.top, 0)):
