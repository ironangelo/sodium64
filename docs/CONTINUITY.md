...
- **Road convergence checkpoint — M2/Gate B closed, M3/Gate C active:** `master@fa943b87b8b59754240270a5cda19d6bc9a59e52` updates only `docs/ROAD_TO_1_0.md` to reconcile the newly returned real-N64 SRS/Nova2 captures with the milestone ladder. No emulator/runtime source changed. Canonical status is now M0/M1/M2 achieved; Gate C is the active gate. The three Gate-B workloads become regression controls. Immediate Gate-C direction: reproduce and isolate the known **SMW iris/window/color-math** and **ALttP rain/tree layer-compositor** fidelity failures using deterministic/reference evidence before touching performance architecture again.
