...
### ACTIVE / RESUME HERE — E2b carrier geometry discriminator dispatched; pixels remain non-authoritative
...
- E2b validation branch is now **`phase4/gate-c-h-comp-e2b-real-target-switch@71737e68a833c118dfbc3c675874036f687ab702`**.
...
- Exact-head **Build and Validate `35663719717 SUCCESS`** on `71737e68...`; build/profile/pinned smoke are green after the one-line guest-only repair. **E2b Carrier Section Proof `35663719699`** remains in progress. Pass criterion is unchanged: record 0 `WH0=0, TS=2, TM=1, split=8`; record 1 `WH0=1, TS=2, TM=1, split=224` in a stable queue, with fresh guest frame, SP HALT and DP idle.
- **Do not interpret target pixels yet.** First require the exact section geometry above plus fresh guest frame, SP HALT and DP idle. A red result means diagnose carrier/queue timing before any further RSP edit.
- Only after carrier geometry validates may the same frozen runtime be judged against the pixel oracle: compact physical rows **0..7**, active x=12..267 = 2,048× green `0x07C1`, compact borders 192× `0x55AA` with guards intact; corresponding main band is **rows 0..7 of the host capture beginning at the published `FRAMEBUFFER(sp)` pointer**, active x=12..267 = 2,048× red `0xF801` and zero green. This row convention follows directly from the runtime target base `FRAMEBUFFER(sp)-8*560`: global RDP y=8 maps to byte 0 at the published framebuffer pointer. Main border contents are not part of this discriminator.
- **Oracle audit after carrier repair:** current RSP binds the compact Color Image before entering `next_section`, so the first section's backdrop fill also lands in compact scratch. That does not change the frozen E2b discriminator: with black backdrop and opaque BG2 green, compact active x=12..267 must finish green; the existing pass-boundary switch occurs before the TM traversal, so main active x=12..267 must finish BG1 red. Main border/background is intentionally excluded because main never receives that first-section backdrop fill. No oracle change required.
- **Framebuffer readback ownership audited:** at `next_frame`, after Sync Full and explicit DP-idle wait, the RSP stores `FRAMEBUFFER(sp)` into the proof marker and only then halts/rotates `sp`. Therefore `proof_framebuffer_pointer` names the framebuffer that just completed rendering, not the next triple-buffer slot. The future E2b main-band classifier can safely use `framebuffer[:0x1180]` from that pointer with no RSP-side archive/copy helper.
- **OPEN QUESTION / next pixel-stage risk:** validated E2a changed/reused Color Image only after a `Sync Full` + explicit DP-idle ownership fence. Current E2b switches compact -> main at the real TS/TM pass boundary by emitting Set Color Image directly, without an equivalent preceding fence. This is not evidence of failure and must not be changed before pixel measurement; if carrier geometry is green but compact/main pixels do not separate, test this ownership/synchronization difference first as a controlled RSP delta.
...
- Status: **E2B RUNTIME IMPLEMENTED CANDIDATE / REPAIRED CARRIER VALIDATION IN FLIGHT / TARGET-SWITCH SEMANTICS NOT YET JUDGED**.
- Immediate next action: inspect exact-head semantic run `35663719699`. If geometry is green, checkpoint carrier validation before changing any pixel classifier; then evaluate the frozen target-switch runtime with a host-only pixel oracle. If geometry is still red, diagnose only guest/queue timing and keep `src/rsp_main.S` frozen.
