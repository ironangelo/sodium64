...
### ACTIVE / RESUME HERE — E2d overrun baseline VALIDATED; section→band repair design next
...
- **Next action:** host-only repair oracle on the same E2d branch: keep carrier split 16/224 and main 4,096× red, but require compact final 2,048× red + 192× sentinel with **both guards fully intact**. This tests bounded reuse of the same 8-row strip after the runtime subloop. Do not add compositor arithmetic, band archive/readback, OBJ cases or production-general scheduling in this batch.
