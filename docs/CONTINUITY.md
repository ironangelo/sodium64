...
- E2b validation branch is now **`phase4/gate-c-h-comp-e2b-real-target-switch@ecd7a2ec4d57afae8c94de15c75f8e85f02182fe`**.
...
- **Independent off-by-one control:** validated H-SAMPLE artifact from run `35539983428` / artifact `10614436889` records `WH0=0 -> end_line 1`, `WH0=1 -> end_line 2`, … `WH0=7 -> end_line 8`, and finishes `WH0=223 -> end_line 224`. Therefore the E2b table's eight initial zero transfers followed by value 1 supports the precommitted `split_line=8` expectation without adapting to E2b output.
- **SUPPORTED STATIC HYPOTHESIS — cross-frame WH0 persistence may falsify the current carrier oracle:** `reload_hdma` resets HDMA mask/table position/counter but does **not** reset WH0; `vblank_end` calls `reload_hdma` then `section_init`, so the first queued section snapshots the prior frame's WH0. `run_line` checks/makes a section **before** `trigger_hdma`, while `write_wh0` sets urgent `sect_status=0x100` only on a value change. The carrier leaves WH0=1 at the end of visible lines and its NMI does not restore it, so warmed frames are predicted to start with WH0=1; line 0 HDMA writes 0 and should create an extra `split_line=0`, followed by the intended `WH0=0 -> split_line=8` transition and final WH0=1 section. This is **not MEASURED yet**; run `35658901047` decides it. If confirmed, repair the guest carrier only by restoring disabled-window WH0=0 during VBlank/NMI; keep the RSP target-switch runtime frozen.
...
- Status: **IMPLEMENTED CANDIDATE / CARRIER VALIDATION IN FLIGHT**.
- Immediate next action: inspect `35658901047` artifact/result first. If carrier geometry passes, checkpoint it before evaluating pixels from the same or a separately isolated capture. If it fails, repair only carrier/harness logic; do not change the target-switch runtime to chase a section-timing failure.
