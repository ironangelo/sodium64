...
This file is the handoff point for continuing the project across ChatGPT conversations, development sessions, and long gaps between milestones. Read it before making architectural changes.

## Project ownership and working model

- Repository: `ironangelo/sodium64`
- Upstream parent: `Hydr8gon/sodium64`
- `master` is our stable integration branch.
- Development happens in branches and pull requests inside **this fork**. Do not open pull requests against the upstream repository unless Iron explicitly asks for that.
- Iron is the project owner and sets the north star. The assistant acts as technical lead/captain: architecture, implementation strategy, profiling, validation, Git workflow, and deciding when real-hardware testing is worth requesting.
...
Important characteristics of the inherited design:
...
- DSP/audio, DMA, memory, input, menu, and related subsystems are also predominantly assembly.
...
The repository contains `docs/VALIDATION.md` with the full policy.
...
PR #1 was a bootstrap: GitHub Actions were still disabled for this newly created public fork, so it could not validate itself before merge.

### GitHub Actions activation

On 15 Sep 2026 Iron manually enabled GitHub Actions for the fork. From this point onward, pull requests should exercise `.github/workflows/autobuild.yml` automatically.

### PR #2 — Phase 1: record baseline measurement map

Status at creation of this continuity file: **open**.
...
- document the current execution/budget model;
- serve as the first PR expected to pass the newly enabled PR build gate;
- correct the fork README so its release link points to `ironangelo/sodium64`, not upstream.

This continuity file itself is being added to PR #2 so its new commit also exercises the freshly enabled Actions configuration.

## Immediate next steps

1. Confirm that PR #2 automatically starts `Build and Validate` now that Actions are enabled.
2. Inspect CI output and build artifacts; fix the workflow if needed.
...
- Update this file whenever project phase, active PR, major hypothesis, key result, or next action materially changes.

## Files to read when resuming

Read these in order:

1. `docs/CONTINUITY.md` — current handoff/state.
2. `docs/ROADMAP.md` — long-term phase plan.
3. `docs/VALIDATION.md` — testing and merge policy.
4. `docs/BASELINE.md` — Phase 1 measurement model once PR #2 is merged.
5. Active PR description/diff and CI results.

If those sources disagree, prefer the most recent concrete repository/PR state and then update this continuity file to remove the inconsistency.
