...
- Status: **E2B RUNTIME IMPLEMENTED CANDIDATE / INITIAL CARRIER REJECTED / TARGET-SWITCH SEMANTICS NOT YET JUDGED**.
- Immediate next action: **guest-only carrier repair** on the same branch: restore disabled-window WH0 to `0` during VBlank/NMI so `vblank_end -> section_init` begins from WH0=0; keep `src/rsp_main.S`, section classifier oracle and target addresses byte-identical. Re-run the exact section-only discriminator. Expected repaired geometry remains record 0 `WH0=0, split=8`, record 1 `WH0=1, split=224`; only after that is green may target pixels become authoritative.
