...
- **Immediate E2c action:** change **only** the proof runtime's shared-membership suppression at the mask pack (stop applying `sub s7,s7,t1`; preserve target switch, guest, carrier, addresses and fences). In a separate host-only oracle update, change compact expectation from 2,048× black `0x0001` to **2,048× red `0xF801`** while keeping main red and all geometry/guards unchanged. If green, E2c shared-membership closes; if red, diagnose layer traversal/cache/priority before any compositor work.
