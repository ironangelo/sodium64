...
- **Next architecture uncertainty:** the validated compact operand is only 8 rows. A real raster section can exceed 8 rows, so current target rebasing will overrun the strip unless the renderer subdivides one loaded section into fixed-height bands. The next experiment must isolate that requirement before compositor arithmetic.
