#!/usr/bin/env python3
"""Capture/classify Gate-C E1f raw RGB555 half-add proof."""
...
E1D_EXPECTED_WORDS = [
    0x0000, 0x0010, 0x0200, 0x4000, 0x7FFF, 0x000F, 0x01E0, 0x3C00,
    0x000F, 0x01E0, 0x318C, 0x0C63, 0x3DEF, 0x3DEF, 0x3DEF, 0x2AAA,
]
...
def wait_for_proof(
    ...
    *,
    ...
    raise RuntimeError("E1f proof marker/fresh guest frame was not observed")
...
def classify(
    ...
    return {
        "classification": (
            "E1F_RGB555_HALF_ADD_VALIDATED"
            if passed
            else "E1F_RGB555_HALF_ADD_FAILED"
        ),
        "passed": passed,
        "constants": {
...
def main() -> int:
    ...
    try:
        ...
        validate_stop(client.request("c"), "E1f clean-epoch breakpoint")
        ...
        validate_stop(client.request("s"), "E1f breakpoint step-over")
        set_breakpoint(client, args.capture_ready_address, True)
        validate_stop(client.request("c"), "E1f fenced capture breakpoint")
        ...
        if not result["passed"]:
            raise RuntimeError("E1f semantic classifier failed; see result.json")
