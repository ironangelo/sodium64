#!/usr/bin/env python3
"""Generate the original Gate-C OBJ-window diagnostic SNES ROM.

This ROM contains no commercial game data. It isolates one Sodium64 Gate-C
question: does the renderer apply the SNES object window when TMW bit 4 enables
it?

Visible setup:
- Mode 1, BG1 + OBJ on main screen only.
- W1 = [64, 191].
- BG1 and OBJ both select inverted W1.
- Three opaque 8x8 OBJ markers at x=16, 96, 224 and y=96.
- No HDMA, IRQ, color math, subscreen, W2, or changing graphics state.

The ROM alternates two 120-frame phases:
- control:   TMW=$01 (BG1 window enabled, OBJ window disabled)
- treatment: TMW=$11 (BG1 + OBJ windows enabled)

Only the TMW OBJ-enable bit changes in PPU state. WRAM $7E0001 mirrors the
current phase value ($01/$11) for deterministic debugger synchronization.

Expected SNES behavior for the chosen inverted W1 setup:
- BG1 is visible only inside x=[64, 191] in both phases.
- Control: all three OBJ markers remain visible.
- Treatment: only the x=96 marker remains visible; x=16 and x=224 are masked.

The diagnostic is intentionally static and narrow. It does not test SMW itself,
HDMA sampling, color math, W2 logic, Mode 7 windows, or full compositor
correctness.
"""
...
def build_program() -> bytes:
    ...
    for value, address in (
        (0x03, 0x2123),          # W12SEL: BG1 uses inverted W1
        ...
        (0x00, 0x2128),          # WH2
        (0x00, 0x2129),          # WH3
        (0x11, 0x212C),          # TM: BG1 + OBJ on main screen
        (0x00, 0x212D),          # TS: no subscreen layers
        (CONTROL_TMW, 0x212E),   # TMW phase starts with BG1 only
...
def build_assets() -> tuple[bytes, bytes, bytes, bytes, bytes]:
    ...
    palette[2:4] = (0x03E0).to_bytes(2, "little")      # BG color 1 = green
