...
- **Current Gate B experiment — SRS representative route v1 RUNNING:** `phase3/gate-b-srs-representative@c0652c7007ccb9886ff014af74f0c057b51ff18f`. Only the deterministic controller sequence changes relative to the validated direct-reference lane: hold Right+Run; every 64 guest `frameCounter` ticks, issue one real B edge and hold B in `current` for 16 frames. No player/entity/collision/game-state bypass is introduced. Direct-SFC run **`35420037005`** remains in progress; same-head Build/Validate **`35420037006` SUCCESS** (normal/PROFILE/smoke green). Discovery run records computed route patch/ROM hashes rather than pretending a pre-known hash. **Question:** does normal authored play progress beyond the old wall (`global x=4250`, local center x=154), preferably into `a1b_stairway`? **Readings:** progression/room transition => candidate representative workload; still blocked/oscillating => reject/adjust input only; build/harness failure => repair harness without drawing gameplay conclusions. Do not run Sodium64 on this route until direct SFC proves it useful.
