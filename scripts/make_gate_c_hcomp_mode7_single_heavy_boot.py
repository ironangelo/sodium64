#!/usr/bin/env python3
"""Boot-first Mode1 -> one-heavy-tile Mode7 -> Mode1 overlay guest.

Proof-only original/homebrew workload. The visible Mode7 section is two lines
(80..82). M7SEL=0x80 enables the renderer's native OOB-empty skip, while
M7A=0x0080 and M7HOFS=M7X=0x03F8 place exactly the first horizontal tile
inside the Mode7 mask. After finish_tile7 advances one 16-pixel tile, the
next transformed X starts at 0x40000 and all remaining tiles skip the heavy
map/char/decode/texture-DMA/RDP path.

WRAM $7E0000..$7E0006 matches the existing third-overlay boot discriminator.
"""
...
def write_pair(asm:Assembler,address:int,value:int)->None:
    emit_lda_sta_abs(asm,value&0xFF,address)
...
def build_program()->bytes:
    ...
    emit_lda_sta_abs(a,M7_OOB_EMPTY,0x211A)

    # BG1HOFS also updates M7HOFS in Sodium64's PPU model.
    write_pair(a,0x210D,M7_HOFS)
    write_pair(a,0x211B,M7_A)
    write_pair(a,0x211F,M7_X)
    # M7Y/B/C/D intentionally remain at reset zero.
    ...
    for value,address in (
        ...
        (0x00,0x212F),(0x00,0x2130),(0x20,0x2131),
        (0xFF,0x2132), # Bright fixed sub color; exercise backdrop math.
    ):
        emit_lda_sta_abs(a,value,address)
    ...
    emit_lda_sta_abs(a,0x20,0x4200)
    ...
    lda_sta_long(a,CONTROL_MODE,0x7E0003)
    emit_lda_sta_abs(a,0x00,0x4200)
    ...
    p=a.finish()
    if a.labels["irq"]!=IRQ_OFFSET:
        raise ValueError("IRQ moved")
    return p
...
def build_rom()->bytes:
    ...
    rom[HEADER:HEADER+21]=b"S64 GATEC 1HEAVY".ljust(21,b" ")
